...
prompt_dir() {
  prompt_segment bright_white black '%~'
}
...
prompt_dotnet() {
  if type -p dotnet >/dev/null; then
    prompt_segment bright_blue black "$vc_icons[dotnet] $(dotnet --version)"
  fi
}

prompt_rbenv() {
  if type -p rbenv >/dev/null; then
    prompt_segment bright_red black "$vc_icons[ruby] $(rbenv version-name)"
  fi
}
